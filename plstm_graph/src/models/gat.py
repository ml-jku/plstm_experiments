...
class GNNModel(BaseModel):
    def __init__(self, task, num_layers, num_features, hidden_dim, num_classes, rand_features, graph_pooling_type,
        ...
        for i in range(num_layers):
            self.node_convs.append(GATConv(hidden_dim, hidden_dim))
    ...
    def forward(self, data, mode=None):
        ...
        # dirchlet energy
        #differences = (x[row] - x[col]).pow(2).sum(dim=1)
        #nodes = scatter(differences, col, 0, x.size(0), reduce='sum')
        #graph = scatter(nodes, batch, 0, batch.max() + 1, reduce='sum')
        #self.log('dirichlet_energy', graph.mean())
