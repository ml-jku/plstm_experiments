...
from .custom_transforms.util import get_rng_from_global
import numpy as np
...
class StochasticCollator(Collator):
...
class MixCollator(StochasticCollator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        mixup_p = mixup_p or 0.0
        cutmix_p = cutmix_p or 0.0

        # check probabilities
        assert isinstance(mixup_p, (int, float)) and 0.0 <= mixup_p <= 1.0, f"invalid mixup_p {mixup_p}"
        assert isinstance(cutmix_p, (int, float)) and 0.0 <= cutmix_p <= 1.0, f"invalid mixup_p {mixup_p}"
        assert 0.0 < mixup_p + cutmix_p <= 1.0, f"0 < mixup_p + cutmix_p <= 1 (got {mixup_p + cutmix_p})"
        if mixup_p + cutmix_p != 1.0:
        ...
        if mixup_p == 0.0:
            ...
            assert isinstance(mixup_alpha, (int, float)) and 0.0 < mixup_alpha
        if cutmix_p == 0.0:
            assert cutmix_alpha is None
        else:
            assert isinstance(cutmix_alpha, (int, float)) and 0.0 < cutmix_alpha
