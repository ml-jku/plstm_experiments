...
class PatchEmbed(ViTBase, nn.Module):
    def setup(self):
        ...
        if self.pooling == "cls":
            self.cls_token = self.param("cls_token", init.truncated_normal(0.02), (1, 1, self.dim))

        if self.posemb == "learnable":
            self.wpe = self.param("wpe", init.truncated_normal(0.02), (*self.num_patches, self.dim))
...
class Attention(ViTBase, nn.Module):
    ...
    def __call__(self, x: Array, deterministic: bool = True) -> Array:
        ...
        z = jnp.einsum("bhqk,bkhd->bqhd", self.drop(nn.softmax(z), deterministic), self.wv(x))
        return self.drop(self.wo(z), deterministic)
...
class FeedForward(ViTBase, nn.Module):
    ...
    def __call__(self, x: Array, deterministic: bool = True) -> Array:
        return self.drop(self.w2(self.drop(nn.gelu(self.w1(x)), deterministic)), deterministic)
...
class ViTLayer(ViTBase, nn.Module):
    ...
    def __call__(self, x: Array, deterministic: bool = True) -> Array:
        x = x + self.drop(self.scale1 * self.attn(self.norm1(x), deterministic), deterministic)
        x = x + self.drop(self.scale2 * self.ff(self.norm2(x), deterministic), deterministic)
        return x
...
class ViT(ViTBase, nn.Module):
    ...
    def __call__(self, x: Array, deterministic: bool = True) -> Array:
        x = self.drop(self.embed(x), deterministic)
